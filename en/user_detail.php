...
<?php }
else if ($request_status_user=='from' AND $staus_show=='Y') {?>
<a type="button" class="btn btn-warning add-relation" data-toggle="modal" data-target="#modal21">Member</a>
<?php }
else{ ?>
...
<a type="button" class="btn btn-warning add-relation" data-toggle="modal" data-target="#contactoption">Add Member</a>
<?php }?>
</div>
</div>
</div>
...
<?php if (isset($_SESSION['user_mid'])) { ?>
<a href="tel:<?php echo $row['mobile']; ?>"><i type="button" class="fas fa-phone-alt mx-2"></i> 
<a href="<?php echo RE_EN_PATH; ?>user_detail_pdf.php?id=<?php echo $row['member_id'];?>"><i type="button" class="fas fa-download mx-2 login-signup"></i></a>
...
<div class="col-md-9 text-uppercase">
<h5><?php if($row['popular_name']!=''){echo $row['popular_name']; }else{echo "NA";}?></h5>
</div>
