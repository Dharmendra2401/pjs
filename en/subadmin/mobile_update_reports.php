...
<?php 
$memberid=mysqli_query($con,'select member_id,first_name,last_name from member where active_status="Y" ');
while($showmemid=mysqli_fetch_array($memberid)){
?>
<option value="<?php echo $showmemid['member_id']; ?>"><?php echo $showmemid['member_id']; ?></option>
<?php } ?>
</select>
</div>
</div>
...
var refrenceid=$("#refrenceid").val();
var memberid=$("#memberidone").val();
 window.location.replace("<?php echo RE_HOME_ADMIN; ?>reports/mobile_updation_reports.php/?state="+ state+"&refrenceid="+ refrenceid+"&memberid="+ memberid);
}
